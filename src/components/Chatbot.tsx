...
const Chatbot = () => {
  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('contact') || lowerMessage.includes('email') || lowerMessage.includes('phone') || lowerMessage.includes('reach')) {
      return "Great! You can reach us at ngumbauNgumbau2@gmail.com or call +254 793 614 592. We're also available on WhatsApp. You can use the contact form on our Contact page here. We typically respond within 2-4 hours during business hours. What's the best way to reach you?";
    }
    ...
    const defaultResponses = [
      ...
      "Thanks for reaching out! We'd love to discuss your project in detail. Our team specializes in cutting-edge technology and creative solutions. Feel free to email us at ngumbauNgumbau2@gmail.com for a comprehensive consultation!",
      ...
      "I'd be happy to provide more specific information about our services! We're experts in modern web technologies, creative design, and data solutions. Let's connect via ngumbauNgumbau2@gmail.com to discuss your project requirements in detail."
    ];
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-16 h-16 rounded-full bg-[var(--brand-primary)] text-white shadow-lg hover:scale-110 transition-all duration-300 flex items-center justify-center z-50 ${isOpen ? 'rotate-180' : ''}`}
      >
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-80 md:w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-40 animate-scale-in">
          {/* Header */}
          <div className="bg-[var(--brand-primary)] text-white p-4 rounded-t-2xl">
            ...
            {messages.map((message, index) => (
              ...
              >
                <div className={`flex items-start space-x-2 max-w-[80%] ${message.isBot ? '' : 'flex-row-reverse space-x-reverse'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${message.isBot ? 'bg-[var(--brand-primary)]' : 'bg-[var(--brand-secondary)]'}`}>
          ...
          <div className="p-4 border-t border-gray-200 bg-white rounded-b-2xl">
            <div className="flex space-x-2">
              <input
                ...
                className="flex-1 p-3 border border-gray-300 rounded-full text-sm focus:outline-none focus:border-[var(--brand-primary)] bg-gray-50 text-black"
                disabled={isTyping}
                style={{ color: '#000000' }}
              />
              <button
                ...
                className="p-3 bg-[var(--brand-primary)] text-white rounded-full hover:scale-105 transition-transform duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Send size={16} />
              </button>
            </div>
